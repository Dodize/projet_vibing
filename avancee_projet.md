...
Au cours de l’avancée sur le projet, nous avons commencé à définir plusieurs outils nous permettant d’être plus efficaces concernant l’utilisation de l’IA. Pour cela, nous avons notamment défini le fichier « AGENTS.md » qui définit les règles devant être respectées. Aussi, nous avons indiqué deux fichiers « project_issues.md » (définissant les tâches à réaliser) et « DATABASE_SCHEMA_REPORT.md » (schéma de base de données) qui doivent être remplis et mis à jour automatiquement par l’IA. Nous avons également créé des commandes : /commit (pour commit le code sur github), /logs (pour indiquer à l’IA de lire le fichier « logs.txt » qui contient les logs présents dans Android studio et /windows_mode qui permet d’indiquer à l’IA que les chemins vers les fichiers doivent être de type windows (utilisation \ au lieu de /).